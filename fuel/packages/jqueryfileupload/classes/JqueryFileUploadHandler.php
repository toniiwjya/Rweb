...
class JqueryFileUploadHandler
{
	...
	protected function get_unique_filename_encrypt($name, $idx=0) {
		$file_path = $this->get_upload_path($name);
		$ext = strtolower(pathinfo($file_path, PATHINFO_EXTENSION));
		$encrypted_name = md5($name.$idx).'.'.$ext;
        if (is_file($this->get_upload_path($encrypted_name))) {
			$idx++;
            return $this->get_unique_filename_encrypt($name, $idx);
        }
    ...
    protected function get_file_name($file_path, $name, $size, $type, $error,
		...
		if (isset($this->options['is_encrypt_filename'])) {
			return $this->get_unique_filename_encrypt($name);
		} else {
